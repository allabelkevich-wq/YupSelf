...
## ПРИНЦИПЫ РАБОТЫ (Karpathy)

4 принципа — как агент должен **думать до того, как писать код**. Законы ниже — реакция на прошлые инциденты; эти принципы — профилактика.

### 1. Думай до кода
- Запрос двусмысленный — **спроси**, не выбирай молча
- Видишь более простой путь — **скажи** об этом, не молчи
- Делай предположения явными, не скрытыми
- Уверен, что подход субоптимален — **пушбэкни**, не соглашайся ради вежливости

### 2. Минимум кода
- Никаких фич «на будущее»
- Никаких абстракций для кода-однодневки
- Никакой гибкости, которую не просили
- Никакой обработки ошибок для сценариев, которые не могут произойти
- Тест: сказал бы senior engineer «переусложнено»? Если да — переписать

### 3. Хирургические правки
- Трогать **только то, что нужно** для задачи
- Не улучшать соседний код «заодно»
- Не рефакторить то, что не сломано
- Соответствовать стилю проекта — даже если ты сделал бы иначе
- Удалять только те импорты/переменные, которые осиротели от **твоих** изменений (не старый мёртвый код)

### 4. Цель → критерий → проверка
- Не «добавь валидацию», а «напиши тест на невалидный инпут, доведи до green»
- Не «почини баг», а «воспроизведи в тесте, потом фикс»
- Сильный критерий успеха = агент работает сам, без постоянных уточнений
- *«LLM отлично делают циклы, пока не достигнут конкретной цели. Не говори что делать — задай критерий успеха»*
